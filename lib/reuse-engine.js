...
// ============================================================================
// FILL DATA - Bulk read/write of reuse groups for centralized data entry
// ============================================================================

async function saveFillData(pool, transactionId, groupUpdates) {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    let totalSaved = 0;

    for (const [groupName, updates] of Object.entries(groupUpdates)) {
      const config = reuseGroups[groupName];
      if (!config || config.readOnly) continue;

      // Only allow columns defined in the reuse group
      const allowedCols = new Set(config.columns);
      const filtered = {};
      for (const [col, val] of Object.entries(updates)) {
        if (allowedCols.has(col)) {
          filtered[col] = (val === '' || val === null || val === undefined) ? null : val;
        }
      }

      if (Object.keys(filtered).length > 0) {
        await writeReuseGroup(client, transactionId, config, groupName, filtered);
        totalSaved += Object.keys(filtered).length;
      }
    }
